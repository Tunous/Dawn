...
class SubmissionSwipeActionsRepository @Inject constructor(
  ...
  fun moveSwipeAction(fromPosition: Int, toPosition: Int, forStartPosition: Boolean) {
    val preference = getPreference(forStartPosition)
    val swipeActions = preference.get().toMutableList()
    val removedSwipeAction = swipeActions.removeAt(fromPosition)
    swipeActions.add(toPosition, removedSwipeAction)
    preference.set(swipeActions)
  }
